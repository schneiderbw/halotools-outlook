...
async function call<T>(path: string, init: RequestInit = {}, retried = false): Promise<T> {
  ...
  try {
    ...
    throw new HaloApiError(
      0,
      `Network call to ${cfg.haloBaseUrl} failed. Most common cause: the Teams app's origin (https://tools.iusehalo.com) is not on this Halo Connect app's CORS allowed origins list. Original error: ${(e as Error).message}`,
    );
  }
